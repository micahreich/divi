...
class EncoderOdom:
    def __init__(self, ticks_per_meter, track_width_m):
        self.TICKS_PER_METER = ticks_per_meter
        self.BASE_WIDTH = track_width_m
...
class Node:
    def __init__(self):
        ...
        self.BASE_WIDTH         = rospy.get_param("~track_width_m", 0.315)
        ...
        rospy.logdebug("ticks_per_rev %f  wheel_diameter_m %f  ticks_per_meter %f",
                       ticks_per_rev, wheel_diameter_m, self.TICKS_PER_METER)
        rospy.logdebug("track_width_m %f", self.BASE_WIDTH)
    ...
    def check_vitals(self, stat):
        ...
        if status in self.ERRORS:
            ...
            matched = [(code, err) for code, err in self.ERRORS.items() if code != 0 and (status & code)]
            if matched:
                state = max(s for _, (s, _) in matched)
                message = ", ".join(msg for _, (_, msg) in matched)
            else:
                state = diagnostic_msgs.msg.DiagnosticStatus.ERROR
                message = "Unknown error: 0x{0:08x}".format(status)
